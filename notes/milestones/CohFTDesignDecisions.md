...
Odd cohomology and Koszul signs are outside the project scope. Products, external products, relabelling, and gluing therefore use ordinary commutativity.
...
Every element has a finite homogeneous decomposition and multiplication sends degrees `i` and `j` to degree `i + j`. A homogeneous element can still be given the type `↥(degree d)`, while a general CohFT class lands directly in the total ring `H g S`.

The intended implementation is a small project-owned package such as `EvenGradedAlgebra R` built on mathlib's `GradedAlgebra`, not a reimplementation of mathlib's grading machinery.
...
This tensor product represents only the even-even Kunneth sector generated by even classes of the two factors. Contributions obtained by tensoring odd classes, including odd-odd contributions of even total degree on the product, are outside the project scope. Thus the interface does not introduce a separate cohomology ring for the product moduli space.

This convention makes the separating CohFT axiom and state-space contraction land directly in the same tensor-product target.
...
- nonseparating gluing gives `H(g+1,S) → H(g,Option (Option S))`;
- separating gluing gives `H(g1+g2,S ⊕ T) → H(g1,Option S) tensor[R] H(g2,Option T)`.

For a label equivalence `e : S ≃ T`, relabelling is instead written in the visible covariant direction

```text
rename e : H(g,S) equivAlg[R] H(g,T).
```

It satisfies `rename (f ∘ e) = rename f ∘ rename e`. This matches `MultilinearMap.domDomCongr e`; the literal contravariant pullback along a relabelling equivalence is recovered as the inverse map.
...
The first stable-curve system contains primitive operations together with the coherence laws directly needed by the initial CohFT interface:
...
The basic stable-curve cohomology system does not include facts special to low genus. A separate `GenusZeroGeometry` extension supplies:

- an algebra identification `H(0, Fin 3) equivAlg[R] R`, expressing that `Mbar(0,3)` is a point;
- the equality, with the necessary label transports, between the boundary pullbacks for the `12|34` and `13|24` presentations of `Mbar(0,4)`.

The first fact turns the three-point CohFT class into an `R`-valued form and is used to extract the Frobenius product. The second supplies the geometric input for associativity and WDVV. Keeping both in an extension lets the basic CohFT and stable-curve interfaces remain independent of special low-genus geometry; the corresponding derived theorems explicitly assume `GenusZeroGeometry`.

### D11. Connected degree-zero cohomology

Projection to degree zero does not by itself produce a scalar-valued topological CohFT. One also needs an identification

```text
degreeZero(H g S) equivAlg[R] R.
```

An optional `ConnectedDegreeZero` extension supplies these identifications for every stable `(g,S)`. It also records that the cohomological unit corresponds to `1 : R` and that relabelling, forgetful pullback, and gluing preserve the corresponding constant classes. The concrete cohomology of stable-curve moduli spaces is expected to carry this extension canonically because the spaces are connected, but the core abstract target does not assume it.

Constructing the scalar-valued topological part of a CohFT explicitly requires `ConnectedDegreeZero`. This makes the hypothesis implicit in the usual literature visible at the precise point where the formalization uses it.

### D12. First acceptance tests

Before building tautological, stable-graph, or GW layers, the M3/M4 architecture must pass the following end-to-end acceptance gate:

1. a constant system `H g S = R`, concentrated in degree zero;
2. a generalization of the existing contraction API from scalar codomain `R` to an arbitrary target module;
3. conversion of the existing `TopologicalCohFT` into the new full CohFT over the constant system;
4. successful relabelling, unit, separating, and nonseparating regression examples, together with recovery of the scalar-valued topological part.
