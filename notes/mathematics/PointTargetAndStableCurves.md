...
[`PointTarget/Descendants.lean`](../../AxiomaticGW/PointTarget/Descendants.lean) implements `PointTarget.primaryClass`, `intersectionNumber`, the point ancestor map, relabelling and dimension-vanishing theorems, the two initial-value interfaces, and the unit-class WDVV check. `PointTarget.DVVRecursion` states the full labelled all-genus formula above using finite subsets for the splitting sum. [`Tautological/LowGenus.lean`](../../AxiomaticGW/Tautological/LowGenus.lean) separately implements the standard genus-zero multinomial value as an executable rational function, verifies the values $\langle\tau_0^3\rangle_0=1$, $\langle\tau_1\tau_0^3\rangle_0=1$, and $\langle\tau_1^2\tau_0^3\rangle_0=2$, and packages agreement with an abstract integration model as `PointTarget.GenusZeroPsiFormula`.
...
The modules deliberately do not manufacture a concrete cohomology ring for $\overline{\mathcal M}_{g,S}$. The tautological package now provides decorated-graph syntax, a free strata module, known-relation quotients, and realization adapters, but these are not a geometric cohomology construction. A future geometric stable-curve model should instantiate `StableCurveCohomology`, `PsiClasses`, and `StableCurveIntegration`, construct the decorated-stratum realization, prove the initial intersections, the required tautological relations, and `DVVRecursion`, and then obtain the numerical point theory from these existing definitions.
